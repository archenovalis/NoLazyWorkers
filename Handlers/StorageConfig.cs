...
namespace NoLazyWorkers.Handlers
{
  ...
  {
    public static Dictionary<string, ConfigPanel> StorageConfigPanelTemplates = new Dictionary<string, ConfigPanel>();
    public static Dictionary<ConfigurableStorageEntity, StorageConfiguration> StorageConfig = new Dictionary<ConfigurableStorageEntity, StorageConfiguration>();
    ...
    public static ConfigPanel SetupConfigPanelTemplate(GameObject storage)
    {
      if (storage == null)
      {
        MelonLogger.Error("SetupConfigPanelTemplate: storage is null");
        return null;
      }
      PlaceableStorageEntity defaultScript = storage.GetComponent<PlaceableStorageEntity>();
      if (defaultScript == null)
      {
        MelonLogger.Error("SetupConfigPanelTemplate: defaultScript is null");
        return null;
      }
      ConfigurableStorageEntity newComponent = storage.AddComponent<ConfigurableStorageEntity>();
      if (newComponent == null)
      {
        MelonLogger.Error("SetupConfigPanelTemplate: newComponent is null");
        return null;
      }
    ...
    public static ConfigPanel GetConfigPanel(string storageType)
    {
      ...
      if (DebugConfig.EnableDebugLogs)
        MelonLogger.Msg($"GetConfigPanel: Template for {storageType} not found after initialization");
      return InitializeStaticTemplate(storageType, StorageConfiguration.GetItemFieldCount(storageType));
    }
  }
  ...
  public class ConfigurableStorageEntity : PlaceableStorageEntity, IConfigurable
  {
    ...
    public override void DestroyItem(bool callOnServer = true)
    {
      if (storageConfiguration != null)
      {
        ...
        StorageExtensions.StorageConfig.Remove(this);
      }
  ...
  public class StorageConfiguration : EntityConfiguration
  {
        ...
        : base(replicator, configurable)
    {
      ...
      StorageExtensions.StorageConfig[storage] = this;
      ...
      try
      {
        ...
        if (gridItem is ConfigurableStorageEntity storageEntity)
        {
          ...
          if (System.IO.File.Exists(configPath))
          {
            ...
            {
              if (StorageExtensions.StorageConfig.TryGetValue(storageEntity, out var config))
              {
  ...
  public class ConfigurableStorageEntityWriteDataPatch
  {
    static void Postfix(ConfigurableStorageEntity __instance, string parentFolderPath, ref List<string> __result)
    {
      try
      {
        if (StorageExtensions.StorageConfig.TryGetValue(__instance, out var config) && config.ShouldSave())
        {
