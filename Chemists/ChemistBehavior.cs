...
namespace NoLazyWorkers_IL2CPP.Chemists
{
  public static class ChemistExtensions
  {
    ...
    public static ItemInstance GetItemInSupply(this Chemist chemist, MixingStation station, string id)
    {
      ...
      if (supplyEntity != null && chemist.behaviour.Npc.Movement.CanGetTo(supplyEntity.TryCast<ITransitEntity>(), 1f))
      {
        list.AddRange(ConvertList(supplyEntity.TryCast<ITransitEntity>().OutputSlots));
  ...
  public class StartMixingStationBehaviourPatch
  {
    ...
    private static bool IsAtSupplies(StartMixingStationBehaviour __instance)
    {
      ...
      bool atSupplies = config != null && supply != null && supply.SelectedObject != null &&
           NavMeshUtility.IsAtTransitEntity(supply.SelectedObject.TryCast<ITransitEntity>(), chemist, 0.4f);
      if (DebugLogs.All || DebugLogs.Chemist)
      {
    ...
    {
      return __instance.targetStation ? __instance.targetStation.TryCast<ITransitEntity>().AccessPoints[0].position : __instance.chemist.transform.position;
    }
  }
