...
using Il2CppPathfinding.Poly2Tri;
...
namespace NoLazyWorkers_IL2CPP.Botanists
{
    ...
    public static void SourceChanged(this PotConfiguration potConfig, BuildableItem item)
    {
      try
      {
        ...
        if ((supply.SelectedObject != null && supply.SelectedObject.TryCast<ITransitEntity>() is ITransitEntity) || item.TryCast<ITransitEntity>() is ITransitEntity)
        {
          ...
          supplyRoute = new TransitRoute(entity, pot.TryCast<ITransitEntity>());
        ...
        try
        {
          if (pot.Configuration is PotConfiguration potConfig)
          {
            ...
            if (!Supply.TryGetValue(guid, out var supply))
            {
              ...
              Supply[guid].onObjectChanged.AddListener(DelegateSupport.ConvertDelegate<UnityAction<BuildableItem>>((BuildableItem item) => SupplyOnObjectChangedInvoke(item, potConfig, pot, supply)));
  ...
  {
    [HarmonyPostfix]
    [HarmonyPatch(MethodType.Constructor, new[] { typeof(ConfigurationReplicator), typeof(IConfigurable), typeof(Pot) })]
    static void ConstructorPostfix(PotConfiguration __instance, Pot pot)
    {
      try
      {
        ...
        supply.onObjectChanged.AddListener(DelegateSupport.ConvertDelegate<UnityAction<BuildableItem>>((BuildableItem item) => PotExtensions.SupplyOnObjectChangedInvoke(item, __instance, pot, supply)));
  ...
  {
    static void Postfix(PotConfigPanel __instance, Il2CppSystem.Collections.Generic.List<EntityConfiguration> configs)
    {
      try
      {
  ...
  public class PotLoaderPatch
  {
    ...
    static void Postfix(string mainPath)
    {
      try
      {
        ...
        if (gridItem.TryCast<Pot>() is not Pot pot)
        {
          if (DebugLogs.All || DebugLogs.Pot)
        ...
        Guid guid = pot.GUID;
        PotConfiguration config = pot.Configuration?.TryCast<PotConfiguration>();
        if (config == null)
        {
